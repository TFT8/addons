...
["AI Systems", "LAMBS Toggle", {
    params [["_position", [0,0,0], [[]], 3], ["_objectUnderCursor", objNull, [objNull]]];
    if (isNull _objectUnderCursor) exitWith {};
    
    private _group = group _objectUnderCursor;
    if !(isNull _group) then {
        private _disable = !(_group getVariable ["lambs_danger_disableGroupAI", false]);
        _group setVariable ["lambs_danger_disableGroupAI", true, true];
        {
            _group setVariable ["lambs_danger_disableAI", true, true];
        } forEach units _group;
        [objNull, format ["LAMBS %1: %2", ["enabled", "disabled"] select _disable, _group]] call bis_fnc_showCuratorFeedbackMessage;
    };
}] call zen_custom_modules_fnc_register;
