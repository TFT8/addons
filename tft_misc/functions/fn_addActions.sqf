...
if(_all || {"BFT" in _actions}) then {
    _object addAction ["<t color='#f0a4ff'>BFT settings</t>", {createDialog "tft_bft_settings"}, nil, -16, false, true, "", "_this distance _target < 8"];
    _object addAction ["<t color='#f0a4ff'>Change Callsign</t>", {createDialog "tft_callsign_settings"}, nil, -18, false, true, "", "_this distance _target < 8"];
};
if(_all || {"Spectator" in _actions}) then {
    _object addAction ["<t color='#83ffc6'>Spectate</t>", {[_this select 1, true] call TFT_fnc_spectate}, nil, -40, false, true, "", "_this distance _target < 10"];
};
